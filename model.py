...
class tranformer(nn.Module):
    def __init__(self, N, d_model, d_ff, d_k, d_v, h, source_vocab, target_vocab, padding_idx, p=0.1) -> None:
        ...
        self.input_embedding = transformer_embedding(source_vocab, d_model, padding_idx)
        self.output_embedding = transformer_embedding(target_vocab, d_model, padding_idx)
        self.encoder = nn.ModuleList([encoder_block(d_model, d_ff, d_k, d_v, h) for _ in range(N)])
        self.decoder = nn.ModuleList([decoder_block(d_model, d_ff, d_k, d_v, h) for _ in range(N)])
    ...
    def padding_mask(self, source, target):
        target = target.ne(self.padding_idx).float()
        source = source.ne(self.padding_idx).float()
